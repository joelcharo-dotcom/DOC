...
export default function CitasPublica() {
  ...
  useEffect(() => {
    // Intenta del servidor, si no del localStorage compartido
    const local = JSON.parse(localStorage.getItem('citas_FUNDAMUFA_v2') || '[]');
    setCitas(local);
    // Si tiene backend API, también intenta cargar
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-blue-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-3xl shadow-2xl p-8 max-w-md w-full text-center">
          ...
          <p className="mt-3 text-gray-700">Dr. Jorge Charrasquiel<br/><b>FUNDAMUFA</b></p>
      ...
      <div className="max-w-xl mx-auto">
        <div className="bg-white rounded-3xl shadow-xl overflow-hidden mt-3 border border-emerald-100">
          <div className="bg-gradient-to-r from-emerald-700 to-emerald-500 text-white p-6 text-center">
            ...
            <p className="opacity-90 text-sm mt-1">FUNDAMUFA - Urología</p>
            <p className="text-xs opacity-80 mt-2 bg-white/20 inline-block px-3 py-1 rounded-full">Agenda tu cita en línea - 24 horas</p>
          </div>
          <div className="p-5">
            ...
            <label className="block text-xs font-bold text-gray-500 uppercase mt-6 mb-3">2️⃣ Elige la hora libre (verde)</label>
            <div className="grid grid-cols-3 gap-2.5">
              {HORARIOS.map(h=>{
                const cita = getCita(h);
                const ocupada = cita?.estado === 'ocupada';
                const bloqueada = cita?.estado === 'bloqueada';
                if(bloqueada) return null; // Paciente NO ve bloqueadas
                const esLibre = !cita;
                return (
                  <button key={h} disabled={ocupada} onClick={()=>esLibre && setSelectedHora(h)}
                    ...
                    ${ocupada ? 'bg-gray-200 border-gray-300 text-gray-400 cursor-not-allowed' : ''}
            ...
            {selectedHora && (
              <div className="mt-6 bg-gray-50 rounded-2xl p-5 border-2 border-emerald-200 animate-in">
                ...
                <p className="text-[10px] text-gray-400 text-center mt-3">Al confirmar aceptas ser contactado por FUNDAMUFA para recordatorio</p>
              </div>
            )}
          </div>
        </div>
        <p className="text-center text-[11px] text-gray-400 mt-4">📍 FUNDAMUFA - Dr. Jorge Charrasquiel<br/>Si tienes emergencia, acude a urgencias</p>
      </div>
    </div>
  );
}
